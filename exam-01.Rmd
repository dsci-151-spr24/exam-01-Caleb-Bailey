...
*I, Caleb Bailey, hereby state that I have not communicated with or gained information in any way from my classmates or anyone other than the Professor or TA during this exam, and that all work is my own.*
...
```{r}
library(dplyr)
flights_data_2013 <- nycflights13::flights %>%
  filter(year == 2013)

top_destinations <- flights_data_2013 %>%
  group_by(dest) %>%
...
airline_flights <- flights_data_2013 %>%
  group_by(carrier) %>%
  summarise(number_of_flights = n()) %>%
  arrange(desc(number_of_flights))
...
non_missing_arrival_delay <- flights_data_2013 %>%
  filter(!is.na(arr_delay))

highest_mean_arrival_delay <- non_missing_arrival_delay %>%
  group_by(carrier) %>%
  summarise(mean_arrival_delay = mean(arr_delay, na.rm = TRUE)) %>%
  arrange(desc(mean_arrival_delay)) %>%
  head(1)

lowest_mean_arrival_delay <- non_missing_arrival_delay %>%
  group_by(carrier) %>%
  summarise(mean_arrival_delay = mean(arr_delay, na.rm = TRUE)) %>%
...
print("Question 3:")
print("a. Carrier with the highest mean arrival delay:")
print(highest_mean_arrival_delay)
print("b. Carrier with the lowest mean arrival delay:")
print(lowest_mean_arrival_delay)
...
highest_departure_delay <- flights_data_2013 %>%
  filter(dep_delay == max(dep_delay, na.rm = TRUE))

mean_temp_highest_dep_delay <- `weather` %>%
  filter(origin %in% highest_departure_delay$origin,
         year == highest_departure_delay$year,
         month == highest_departure_delay$month,
         day == highest_departure_delay$day) %>%
  summarise(mean_temp = mean(temp, na.rm = TRUE))

print("Question 4:")
print("Mean temperature at the origin airport on the day with the highest departure delay:")
print(mean_temp_highest_dep_delay)
...
flights_data_2013 <- flights_data_2013 %>%
  mutate(dep_time_interval = case_when(
    dep_time >= 1 & dep_time <= 600 ~ "12:01am-6am",
    dep_time > 600 & dep_time <= 1200 ~ "6:01am-12pm",
    dep_time > 1200 & dep_time <= 1800 ~ "12:01pm-6pm",
    dep_time > 1800 & dep_time <= 2400 ~ "6:01pm-12am",
    TRUE ~ NA_character_
  ))

delayed_flights_by_interval <- flights_data_2013 %>%
  group_by(dep_time_interval) %>%
  summarise(prop_delayed = mean(dep_delay > 0, na.rm = TRUE))

print("Question 5:")
print("a. Proportion of flights delayed at each time interval:")
print(delayed_flights_by_interval)
...
```
    ...
    ***The later in the day it is, the more likely your flight will be delayed.***
...
longest_air_time_flight <- flights_data_2013 %>%
  arrange(desc(air_time)) %>%
  slice(1)

print("Question 6:")
print("a. Duration of the longest air time flight:")
print(longest_air_time_flight$air_time)


print("b. City to which the longest air time flight flew:")




print("c. Number of seats of the plane that flew the longest air time flight:")
